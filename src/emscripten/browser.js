...
module.exports = function(Module, assert) {
  const GL = require('./system/GL')(Module);
  const Browser = {
    mainLoop: {
      scheduler: null,
      method: '',
    ...
    safeCallback(func) {
      return function(...args) {
        if (!Module.ABORT) return func(...args);
      };
    },
    ...
    Browser.mainLoop.runner = function Browser_mainLoop_runner() {
      ...
      Browser.mainLoop.runIter(() => {
        if (typeof arg !== 'undefined') {
          Module.Runtime.dynCall('vi', func, [arg]);
        } else {
          Module.Runtime.dynCall('v', func);
        }
      });
      if (thisMainLoopId < Browser.mainLoop.currentlyRunningMainloop) return;
      if (typeof Module.SDL === 'object' && Module.SDL.audio && Module.SDL.audio.queueNewAudioData) {
        Module.SDL.audio.queueNewAudioData();
      }
      Browser.mainLoop.scheduler();
    };
  ...
  Browser._emscripten_set_main_loop = _emscripten_set_main_loop;
  Browser._emscripten_set_main_loop_timing = _emscripten_set_main_loop_timing;
