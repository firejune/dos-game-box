...
(function(Promise) {
  ...
  function Emulator(canvas, callback, loadFiles) {
    ...
    var draw_loading_status = function(deltaT) {
      ...
      context.rotate(splash.spinning ? (splash.spinner_rotation += 2 * (2 * Math.PI / 1000) * deltaT) : 0);
    ...
    function getfullscreenenabler() {
      return canvas.webkitRequestFullScreen || canvas.requestFullScreen;
    }
  ...
  window.animLoop = function(render, element) {
    ...
    function loop(now) {
      if (running !== false) {
        ...
        raf ? raf(loop, element) : setTimeout(loop, 1000 / 60);
...
function setup_jsmess_webaudio() {
  ...
  var jsmess_web_audio = (function() {
    ...
    function tick() {
      ...
      var insert_point = (buffer_insert_point === null) ? now : buffer_insert_point;
