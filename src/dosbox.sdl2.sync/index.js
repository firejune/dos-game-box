...
module.exports = function(Module) {
  ...
  const canvasStyle = Module.canvas.style;
  ...
  const sourceRegex = /\(([^)]*)\)\s*{\s*([^*]*?)[\s;]*(?:return\s*(.*?)[;\s]*)?}$/;
...

  {
    const JSfuncs = {
      ...
      arrayToC(arr) {
        const ret = Runtime.stackAlloc(arr.length);
    ...
    Module.ccall = function ccallFunc(ident, returnType, argTypes, args, opts) {
    ...
    Module.cwrap = function cwrap(ident, returnType, argTypes) {
      ...
      if (!numericArgs) {
        // console.log('JSsource.stackSave.body', JSsource.stackSave.body);
        funcstr += 'const stack = ' + JSsource.stackSave.body + ';';
        for (let i = 0; i < nargs; i++) {
          const arg = argNames[i];
          const type = argTypes[i];
          if (type === 'number') continue;
          const convertCode = JSsource[type + 'ToC'];
          funcstr += 'const ' + convertCode.arguments + ' = ' + arg + ';';
      ...
      if (!numericRet) {
        const strgfy = parseJSFunc(() => Pointer_stringify).returnValue;
  ...
  function Pointer_stringify(ptr, length) {
    ...
    if (hasUtf < 128) {
      ...
      while (length > 0) {
        const curr = String.fromCharCode.apply(String, HEAPU8.subarray(ptr, ptr + Math.min(length, MAX_CHUNK)));
  ...
  function demangle(func) {
    const hasLibcxxabi = !!Module.___cxa_demangle;
    if (hasLibcxxabi) {
      let buf;
      let status;
      let ret;
      try {
        buf = _malloc(func.length);
        writeStringToMemory(func.substr(1), buf);
        status = _malloc(4);
        ret = Module.___cxa_demangle(buf, 0, 0, status);
    ...
    function parseNested() {
      ...
      while (func[i] !== 'E') {
        if (func[i] === 'S') {
          i++;
          const next = func.indexOf('_', i);
          const num = func.substring(i, next) || 0;
          parts.push(subs[num] || '?');
          i = next + 1;
          continue;
        }
        ...
        const size = parseInt(func.substr(i), 10);
        const pre = size.toString().length;
        if (!size || !pre) {
          i--;
          break;
        }

        const curr = func.substr(i + pre, size);
        parts.push(curr);
        subs.push(curr);
        i += pre + size;
      }
    ...
    function parse(rawList, limit, allowVoid) {
      ...
      if (func[i] === 'I') {
        i++;
        const iList = parse(true);
        const iRet = parse(true, 1, true);
        ...
        if (c in basicTypes) {
          ...
          switch (c) {
            ...
            case 'L':
              {
                i++;
                const end = func.indexOf('E', i);
                const size = end - i; list.push(func.substr(i, size)); i += size + 2;
                break;
              }

            case 'A':
              {
                const size = parseInt(func.substr(i), 10); i += size.toString().length;
  ...
  const HEAP8 = Module.HEAP8 = new Int8Array(Module.buffer);
  const HEAP16 = Module.HEAP16 = new Int16Array(Module.buffer);
  const HEAP32 = Module.HEAP32 = new Int32Array(Module.buffer);
  const HEAPU8 = Module.HEAPU8 = new Uint8Array(Module.buffer);
  const HEAPU16 = Module.HEAPU16 = new Uint16Array(Module.buffer);
  const HEAPU32 = Module.HEAPU32 = new Uint32Array(Module.buffer);
  const HEAPF32 = Module.HEAPF32 = new Float32Array(Module.buffer);
  const HEAPF64 = Module.HEAPF64 = new Float64Array(Module.buffer);
  HEAP32[0] = 255;
  ...
  const ASM_CONSTS = [function($0, $1, $2) {
    const w = $0;
    const h = $1;
    const pixels = $2;
    ...
    if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) {
      ...
      const data32 = SDL2.data32;
      num = data32.length;
      data32.set(HEAP32.subarray(src, src + num));
      const data8 = SDL2.data8;
      let i = 3;
      const j = i + 4 * num;
      if (num % 8 === 0) {
        while (i < j) {
    ...
    const numChannels = SDL2.audio.currentOutputBuffer.numberOfChannels;
    for (let c = 0; c < numChannels; ++c) {
      const channelData = SDL2.audio.currentOutputBuffer.getChannelData(c);
  ...
  STATIC_BASE = 8;
  STATICTOP = STATIC_BASE + 30125344;
  __ATINIT__.push(
    { func: () => Module.__GLOBAL__sub_I_cpu_cpp() },
    { func: () => Module.__GLOBAL__sub_I_dos_memory_cpp() },
    { func: () => Module.__GLOBAL__sub_I_dos_misc_cpp() },
    { func: () => Module.__GLOBAL__sub_I_drives_cpp() },
    { func: () => Module.__GLOBAL__sub_I_hardware_cpp() },
    { func: () => Module.__GLOBAL__sub_I_vga_memory_cpp() },
    { func: () => Module.__GLOBAL__sub_I_sdl_mapper_cpp() },
    { func: () => Module.__GLOBAL__sub_I_messages_cpp() },
    { func: () => Module.__GLOBAL__sub_I_programs_cpp() },
    { func: () => Module.__GLOBAL__sub_I_setup_cpp() },
    { func: () => Module.__GLOBAL__sub_I_shell_cpp() },
    { func: () => Module.__GLOBAL__sub_I_shell_misc_cpp() }
  );

  memoryInitializer = 'dosbox.html.mem';
  const tempDoublePtr = Runtime.alignMemory(allocate(12, 'i8', ALLOC_STATIC), 8);
  assert(tempDoublePtr % 8 === 0);

  const EMTSTACKTOP = getMemory(1048576);
  const EMT_STACK_MAX = EMTSTACKTOP + 1048576;
  const eb = getMemory(87144);
  __ATPRERUN__.push(require('./memory')(eb, HEAPU8, HEAPU32));
  ...
  const JSEvents = {
    ...
    registerKeyEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.keyEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    getBoundingClientRectOrZeros(target) {
    ...
    fillMouseEventData(eventStruct, e, target) {
      ...
      if (target) {
        const rect = JSEvents.getBoundingClientRectOrZeros(target);
    ...
    registerMouseEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.mouseEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerWheelEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      if (!JSEvents.wheelEvent) {
        JSEvents.wheelEvent = _malloc(104);
      }

      target = JSEvents.findEventTarget(target);
      const wheelHandlerFunc = function(event) {
        const e = event || window.event;
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.wheelEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };

      const mouseWheelHandlerFunc = function(event) {
        const e = event || window.event;
        JSEvents.fillMouseEventData(JSEvents.wheelEvent, e, target);
    ...
    registerUiEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      if (!JSEvents.uiEvent) {
      ...
      const handlerFunc = function(event) {
        ...
        const scrollPos = JSEvents.pageScrollPos();
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.uiEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerFocusEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.focusEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerDeviceOrientationEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.deviceOrientationEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerDeviceMotionEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.deviceMotionEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    fillOrientationChangeEventData(eventStruct, e) {
      const orientations = ['portrait-primary', 'portrait-secondary', 'landscape-primary', 'landscape-secondary'];
      const orientations2 = ['portrait', 'portrait', 'landscape', 'landscape'];
      const orientationString = JSEvents.screenOrientation();
      let orientation = orientations.indexOf(orientationString);
      if (orientation === -1) {
    ...
    registerOrientationChangeEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.orientationChangeEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    fillFullscreenChangeEventData(eventStruct, e) {
      ...
      const id = reportedElement && reportedElement.id ? reportedElement.id : '';
    ...
    registerFullscreenChangeEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.fullscreenChangeEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    resizeCanvasForFullscreen(target, strategy) {
      const restoreOldStyle = __registerRestoreOldStyle(target);
      let cssWidth = strategy.softFullscreen ? window.innerWidth : screen.width;
      let cssHeight = strategy.softFullscreen ? window.innerHeight : screen.height;
      const rect = target.getBoundingClientRect();
      const windowedCssWidth = rect.right - rect.left;
      const windowedCssHeight = rect.bottom - rect.top;
      const windowedRttWidth = target.width;
      const windowedRttHeight = target.height;
      if (strategy.scaleMode === 3) {
        ...
        if (cssWidth * windowedRttHeight < windowedRttWidth * cssHeight) {
          const desiredCssHeight = windowedRttHeight * cssWidth / windowedRttWidth;
          __setLetterbox(target, (cssHeight - desiredCssHeight) / 2, 0);
          cssHeight = desiredCssHeight;
        } else {
          const desiredCssWidth = windowedRttWidth * cssHeight / windowedRttHeight;
    ...
    requestFullscreen(target, strategy) {
    ...
    fillPointerlockChangeEventData(eventStruct, e) {
      const pointerLockElement = document.pointerLockElement || document.webkitPointerLockElement;
      const isPointerlocked = !!pointerLockElement;
      HEAP32[eventStruct >> 2] = isPointerlocked;
      const nodeName = JSEvents.getNodeNameForTarget(pointerLockElement);
      const id = pointerLockElement && pointerLockElement.id ? pointerLockElement.id : '';
      writeStringToMemory(nodeName, eventStruct + 4);
      writeStringToMemory(id, eventStruct + 132);
    },

    registerPointerlockChangeEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.pointerlockChangeEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    fillVisibilityChangeEventData(eventStruct, e) {
    ...
    registerVisibilityChangeEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.visibilityChangeEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerTouchEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        let ptr = JSEvents.touchEvent;
        HEAP32[ptr + 4 >> 2] = e.ctrlKey;
        HEAP32[ptr + 8 >> 2] = e.shiftKey;
        HEAP32[ptr + 12 >> 2] = e.altKey;
        HEAP32[ptr + 16 >> 2] = e.metaKey;
        ptr += 20;
        const canvasRect = Module.canvas ? Module.canvas.getBoundingClientRect() : undefined;
        const targetRect = JSEvents.getBoundingClientRectOrZeros(target);
        let numTouches = 0;
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.touchEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerGamepadEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.gamepadEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerBeforeUnloadEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      const handlerFunc = function(event) {
        const e = event || window.event;
        let confirmationMessage = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, 0, userData]);
    ...
    registerBatteryEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        ...
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, JSEvents.batteryEvent, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
    ...
    registerWebGlEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) {
      ...
      const handlerFunc = function(event) {
        const e = event || window.event;
        const shouldCancel = Runtime.dynCall('iiii', callbackfunc, [eventTypeId, 0, userData]);
        if (shouldCancel) {
          e.preventDefault();
        }
      };
  ...
  const GL = {
    ...
    initExtensions(context) {
      ...
      if (context.version < 2) {
        const instancedArraysExt = GLctx.getExtension('ANGLE_instanced_arrays');
        ...
        const drawBuffersExt = GLctx.getExtension('WEBGL_draw_buffers');
        if (drawBuffersExt) {
    ...
    populateUniformTable(program) {
      ...
      const numUniforms = GLctx.getProgramParameter(p, GLctx.ACTIVE_UNIFORMS);
  ...
  function _emscripten_glIsRenderbuffer(renderbuffer) {
    const rb = GL.renderbuffers[renderbuffer];
    if (!rb) return 0;
  ...
  function _dlsym(handle, symbol) {
    ...
    const lib = DLFCN.loadedLibs[handle];
    symbol = '_' + symbol;
  ...
  function _dlerror() {
    ...
    if (DLFCN.error) Module._free(DLFCN.error);
    const msgArr = intArrayFromString(DLFCN.errorMsg);
  ...
  const NODEFS = {
    ...
    flagsToPermissionStringMap: {
      0: 'r', 1: 'r+', 2: 'r+', 64: 'r', 65: 'r+', 66: 'r+', 129: 'rx+', 193: 'rx+', 514: 'w+',
      577: 'w', 578: 'w+', 705: 'wx', 706: 'wx+', 1024: 'a', 1025: 'a', 1026: 'a+', 1089: 'a',
      1090: 'a+', 1153: 'ax', 1154: 'ax+', 1217: 'ax', 1218: 'ax+', 4096: 'rs', 4098: 'rs+'
    },
  ...
  */
  const FS = {
    ...
    createLazyFile(parent, name, url, canRead, canWrite) {
      ...
      LazyUint8Array.prototype.cacheLength = function LazyUint8Array_cacheLength() {
        ...
        const hasByteServing = header && header === 'bytes';
    ...
    createPreloadedFile(parent, name, url, canRead, canWrite, onload, onerror, dontCreateFile, canOwn, preFinish) {
      ...
      if (typeof url === 'string') {
        Browser.asyncLoad(url, byteArray => {
    ...
    try {
      ...
      switch (op) {
        ...
        case 21519:
          {
            if (!stream.tty) return -ERRNO_CODES.ENOTTY;
            const argp = SYSCALLS.get(); HEAP32[argp >> 2] = 0;
            return 0;
          }
  ...
  function _localtime_r(time, tmPtr) {
    ...
    const zonePtr = HEAP32[_tzname + (dst ? Runtime.QUANTUM_SIZE : 0) >> 2];
  ...
  function _dlclose(handle) {
    ...
    const lib_record = DLFCN.loadedLibs[handle];
    if (--lib_record.refcount === 0) {
      if (lib_record.module.cleanups) {
        lib_record.module.cleanups.forEach(cleanup => {
          cleanup();
        });
      }
  ...
  const _emscripten_GetProcAddress = undefined;
  ...
  function _emscripten_set_main_loop_timing(mode, value) {
    ...
    if (mode === 0) {
      ...
      if (!window.setImmediate) {
        const setImmediates = [];
        const emscriptenMainLoopMessageId = '__emcc';
  ...
  const Browser = {
    ...
    init() {
      ...
      Browser.BlobBuilder = typeof WebKitBlobBuilder !== 'undefined' ? WebKitBlobBuilder : null;
      Browser.URLObject = window.URL;
    ...
    requestFullScreen(lockPointer, resizeCanvas, vrDevice) {
    ...
    setFullScreenCanvasSize() {
      if (typeof SDL !== 'undefined') {
        let flags = HEAPU32[SDL.screen + Runtime.QUANTUM_SIZE * 0 >> 2];
        flags = flags | 8388608;
  ...
  function emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, internalFormat) {
    ...
    let numChannels;
    switch (format) {
      case 6406: case 6409: case 6402: case 6403:
        numChannels = 1;
        break;
      case 6410: case 33319:
        numChannels = 2;
        break;
      case 6407: case 35904:
        numChannels = 3;
        break;
      case 6408: case 35906:
        numChannels = 4;
        break;
      default:
        GL.recordError(1280);
    ...
    switch (type) {
      ...
      case 34042:
        sizePerPixel = 4;
        break;
      case 33635: case 32819: case 32820:
        sizePerPixel = 2;
        break;
      default:
        GL.recordError(1280);
    ...
    const bytes = emscriptenWebGLComputeImageSize(width, height, sizePerPixel, GL.unpackAlignment);
    if (type === 5121) {
  ...
  function emscriptenWebGLGetVertexAttrib(index, pname, params, type) {
    ...
    const data = GLctx.getVertexAttrib(index, pname);
  ...
  function _emscripten_do_request_fullscreen(target, strategy) {
    ...
    const canPerformRequests = JSEvents.canPerformEventHandlerRequests();
  ...
  let __currentFullscreenStrategy = {};
  ...
  function __registerRestoreOldStyle(canvas) {
    ...
    const oldBackgroundColor = canvas.style.backgroundColor;
    const oldDocumentBackgroundColor = document.body.style.backgroundColor;
    const oldPaddingLeft = canvas.style.paddingLeft;
    ...
    const oldMarginLeft = canvas.style.marginLeft;
    ...
    const oldDocumentScroll = document.body.scroll;
    ...
    if (pname === 35716) {
      ...
      if (ptable) {
        if (ptable.maxAttributeLength === -1) {
          ...
          const numAttribs = GLctx.getProgramParameter(program, GLctx.ACTIVE_ATTRIBUTES);
          ptable.maxAttributeLength = 0;
          for (let i = 0; i < numAttribs; ++i) {
            const activeAttrib = GLctx.getActiveAttrib(program, i);
  ...
  function _sysconf(name) {
    switch (name) {
      ...
      case 132: case 133: case 12: case 137: case 138: case 15: case 235: case 16: case 17: case 18:
      case 19: case 20: case 149: case 13: case 10: case 236: case 153: case 9: case 21: case 22:
      case 159: case 154: case 14: case 77: case 78: case 139: case 80: case 81: case 82: case 68:
      case 67: case 164: case 11: case 29: case 47: case 48: case 95: case 52: case 51: case 46:
        return 200809;
      case 79:
        return 0;
      case 27: case 246: case 127: case 128: case 23: case 24: case 160: case 161: case 181:
      case 182: case 242: case 183: case 184: case 243: case 244: case 245: case 165: case 178:
      case 179: case 49: case 50: case 168: case 169: case 175: case 170: case 171: case 172:
      case 97: case 76: case 32: case 173: case 35:
        return -1;
      case 176: case 177: case 7: case 155: case 8: case 157: case 125: case 126: case 92: case 93:
      case 129: case 130: case 131: case 94: case 91:
        return 1;
  ...
  function _strftime(s, maxsize, format, tm) {
    ...
    const EXPANSION_RULES_2 = {
      ...
      '%C': _date => leadingNulls(_date.tm_year + 1900 / 100 | 0, 2),
      ...
      '%j': _date => leadingNulls(_date.tm_mday + __arraySum(__isLeapYear(_date.tm_year + 1900) ? __MONTH_DAYS_LEAP : __MONTH_DAYS_REGULAR, _date.tm_mon - 1), 3),
      '%m': _date => leadingNulls(_date.tm_mon + 1, 2),
      '%M': _date => leadingNulls(_date.tm_min, 2),
      '%n': () => '\n',
      '%p': _date => _date.tm_hour > 0 && _date.tm_hour < 13 ? 'AM' : 'PM',
      '%S': _date => leadingNulls(_date.tm_sec, 2),
      '%t': () => '\t',
      '%u': _date => (new Date(_date.tm_year + 1900, _date.tm_mon + 1, _date.tm_mday, 0, 0, 0, 0)).getDay() || 7,
      '%U': _date => {
      ...
      '%w': _date => (new Date(_date.tm_year + 1900, _date.tm_mon + 1, _date.tm_mday, 0, 0, 0, 0)).getDay(),
      '%W': _date => {
  ...
  function _emscripten_glDeleteRenderbuffers(n, renderbuffers) {
    for (let i = 0; i < n; i++) {
      const id = HEAP32[renderbuffers + i * 4 >> 2];
      const renderbuffer = GL.renderbuffers[id];
  ...
  function emscriptenWebGLGetUniform(program, location, params, type) {
    ...
    const data = GLctx.getUniform(GL.programs[program], GL.uniforms[location]);
    if (typeof data === 'number' || typeof data === 'boolean') {
      switch (type) {
        case 'Integer':
          HEAP32[params >> 2] = data;
  ...
  function _emscripten_glGetShaderPrecisionFormat(shaderType, precisionType, range, precision) {
    const result = GLctx.getShaderPrecisionFormat(shaderType, precisionType);
  ...
  function _emscripten_glGetActiveAttrib(program, index, bufSize, length, size, type, name) {
    program = GL.programs[program];
    const info = GLctx.getActiveAttrib(program, index);
    if (!info) return;
    const infoname = info.name.slice(0, Math.max(0, bufSize - 1));
    if (bufSize > 0 && name) {
  ...
  function _emscripten_glIsFramebuffer(framebuffer) {
    const fb = GL.framebuffers[framebuffer];
    if (!fb) return 0;
  ...
  function _emscripten_glGetFramebufferAttachmentParameteriv(target, attachment, pname, params) {
    const result = GLctx.getFramebufferAttachmentParameter(target, attachment, pname);
  ...
  const GLUT = {
    ...
    onMousemove(event) {
      const lastX = Browser.mouseX;
      const lastY = Browser.mouseY;
      Browser.calculateMouseEvent(event);
      const newX = Browser.mouseX;
      const newY = Browser.mouseY;
    ...
    getSpecialKey(keycode) {
      ...
      switch (keycode) {
        ...
        case 112: key = 1; break;
        case 113: key = 2; break;
        case 114: key = 3; break;
        case 115: key = 4; break;
        case 116: key = 5; break;
        case 117: key = 6; break;
        case 118: key = 7; break;
        case 119: key = 8; break;
        case 120: key = 9; break;
        case 121: key = 10; break;
        case 122: key = 11; break;
        case 123: key = 12; break;
        case 37: key = 100; break;
        case 38: key = 101; break;
        case 39: key = 102; break;
        case 40: key = 103; break;
        case 33: key = 104; break;
        case 34: key = 105; break;
        case 36: key = 106; break;
        case 35: key = 107; break;
        case 45: key = 108; break;
        case 16:
        case 5: key = 112; break;
        case 6: key = 113; break;
        case 17:
        case 3: key = 114; break;
        case 4: key = 115; break;
        case 18:
        case 2: key = 116; break;
        case 1: key = 117; break;
      }
      return key;
    },
    ...
    getASCIIKey(event) {
      ...
      if (keycode >= 65 && keycode <= 90) return event.shiftKey ? keycode : keycode + 32;
      if (keycode >= 96 && keycode <= 105) return keycode - 48;
      if (keycode >= 106 && keycode <= 111) return keycode - 106 + 42;
      switch (keycode) {
        case 9: case 13: case 27: case 32: case 61:
          return keycode;
      }
    ...
    onKeydown(event) {
      if (GLUT.specialFunc || GLUT.keyboardFunc) {
        let key = GLUT.getSpecialKey(event.keyCode);
        if (key !== null) {
          if (GLUT.specialFunc) {
            event.preventDefault();
            GLUT.saveModifiers(event);
    ...
    requestFullScreen() {
      const RFS = Module.canvas.requestFullscreen || Module.canvas.requestFullScreen || Module.canvas.webkitRequestFullScreen || function() {};

      RFS.apply(Module.canvas, []);
    },

    cancelFullScreen() {
      const CFS = document.exitFullscreen || document.cancelFullScreen || document.webkitCancelFullScreen || function() {};
  ...
  function _eglCreateContext(display, config, hmm, contextAttribs) {
    ...
    let glesContextVersion = 1;
    for (;;) {
      const param = HEAP32[contextAttribs >> 2];
      if (param === 12440) {
  ...
  function _emscripten_glReadPixels(x, y, width, height, format, type, pixels) {
    const data = emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, format);
    if (!data.pixels) {
      GL.recordError(1280);
      return;
    }
  ...
  function _emscripten_glBindBuffer(target, buffer) {
    const bufferObj = buffer ? GL.buffers[buffer] : null;
  ...
  function _emscripten_glGetAttachedShaders(program, maxCount, count, shaders) {
    const result = GLctx.getAttachedShaders(GL.programs[program]);
    ...
    HEAP32[count >> 2] = len;
    for (let i = 0; i < len; ++i) {
      const id = GL.shaders.indexOf(result[i]);
  ...
  function _emscripten_glGenRenderbuffers(n, renderbuffers) {
    for (let i = 0; i < n; i++) {
      ...
      const id = GL.getNewId(GL.renderbuffers);
  ...
  const EmterpreterAsync = {
    ...
    handle(doAsyncOp, yieldDuring) {
      Module.noExitRuntime = true;
      if (EmterpreterAsync.state === 0) {
        const stack = new Int32Array(HEAP32.subarray(EMTSTACKTOP >> 2, asm.emtStackSave() >> 2));
        // const stacktop =
        asm.stackSave();
        let resumedCallbacksForYield = false;

        function resumeCallbacksForYield() {
          if (resumedCallbacksForYield) return;
          resumedCallbacksForYield = true;
          EmterpreterAsync.yieldCallbacks.forEach(func => {
            func();
          });
        ...
        doAsyncOp(function resume(post) {
          ...
          if (EmterpreterAsync.state === 0) {
            EmterpreterAsync.asyncFinalizers.forEach(func => {
              func();
            });
        ...
        if (EmterpreterAsync.postAsync) {
          const ret = EmterpreterAsync.postAsync();
  ...
  function _emscripten_glGetActiveUniform(program, index, bufSize, length, size, type, name) {
    program = GL.programs[program];
    const info = GLctx.getActiveUniform(program, index);
    if (!info) return;
    const infoname = info.name.slice(0, Math.max(0, bufSize - 1));
    if (bufSize > 0 && name) {
      writeStringToMemory(infoname, name);
  ...
  function _emscripten_glGenBuffers(n, buffers) {
    for (let i = 0; i < n; i++) {
      ...
      const id = GL.getNewId(GL.buffers);
      buffer.name = id;
  ...
  function _emscripten_glGetUniformLocation(program, name) {
    ...
    if (name.indexOf(']', name.length - 1) !== -1) {
      const ls = name.lastIndexOf('[');
      const arrayIndex = name.slice(ls + 1, -1);
      if (arrayIndex.length > 0) {
        arrayOffset = parseInt(arrayIndex, 10);
        if (arrayOffset < 0) {
          return -1;
        }
      }
  ...
  function _emscripten_glDrawBuffers(n, bufs) {
    const bufArray = [];
    for (let i = 0; i < n; i++) bufArray.push(HEAP32[bufs + i * 4 >> 2]);
  ...
  function _sbrk(bytes) {
    ...
    const ret = DYNAMICTOP;
    if (bytes !== 0) {
      const success = self.alloc(bytes);
  ...
  function _malloc(bytes) {
    const ptr = Runtime.dynamicAlloc(bytes + 8);
  ...
  function _emscripten_glTexImage2D(target, level, internalFormat, width, height, border, format, type, pixels) {
    let pixelData;
    if (pixels) {
      const data = emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, internalFormat);
  ...
  function ___syscall221(which, varargs) {
    ...
    try {
      ...
      switch (cmd) {
        ...
        case 4:
          {
            const arg = SYSCALLS.get(); stream.flags |= arg;
            return 0;
          }
        ...
        case 12:
          {
            const arg = SYSCALLS.get();
            const offset = 0; HEAP16[arg + offset >> 1] = 2;
            return 0;
          }
  ...
  const _sin = Math.sin;
  const _log = Math.log;
  const _environ = allocate(1, 'i32*', ALLOC_STATIC);
  ...
  Module.asmLibraryArg = {
    abort, assert, invoke_iiiiiiii, invoke_iiiiiid, invoke_vd, invoke_vid, invoke_viiiii, invoke_vi,
    invoke_vii, invoke_iiiiiii, invoke_ii, invoke_viiiiiiiiiii, invoke_viddd, invoke_iiiii,
    invoke_vidd, invoke_iiii, invoke_viiiiiiii, invoke_viiiiii, invoke_viii, invoke_viid,
    invoke_vidddd, invoke_iiiiiiiii, invoke_viiiiiii, invoke_viiiiiiiii, invoke_iii, invoke_iiiiii,
    invoke_i, invoke_iiiiidii, invoke_iiiiiiiiii, invoke_vdddddd, invoke_vdddd, invoke_vdd,
    invoke_v, invoke_vdi, invoke_iiiiid, invoke_viiii, _emscripten_glGetTexParameterfv,
    _emscripten_glDeleteVertexArrays, _emscripten_set_mouseleave_callback, _strftime_l,
    _emscripten_glShaderSource, _emscripten_asm_const_3, _emscripten_glReleaseShaderCompiler,
    _emscripten_glBlendFuncSeparate, _emscripten_glCopyTexImage2D, _dlsym,
    _emscripten_glGetShaderPrecisionFormat, _emscripten_glGetIntegerv, _emscripten_glCullFace,
    ___cxa_guard_acquire, _emscripten_set_keyup_callback, _emscripten_glStencilMaskSeparate,
    _emscripten_glViewport, _emscripten_glFrontFace, _eglTerminate, ___assert_fail,
    _emscripten_glDrawArrays, _emscripten_glUniform3fv, __ZSt18uncaught_exceptionv,
    _emscripten_glUseProgram, __setLetterbox, _emscripten_glDepthFunc, __addDays,
    _emscripten_set_fullscreenchange_callback, _signal, _emscripten_set_touchmove_callback,
    _emscripten_set_main_loop_timing, _emscripten_set_gamepaddisconnected_callback, _sbrk,
    _emscripten_glDisableVertexAttribArray, _emscripten_glUniform3iv, ___cxa_begin_catch,
    _emscripten_glGetProgramiv, _eglBindAPI, _sysconf, _execl, _eglCreateContext,
    _emscripten_glVertexPointer, _emscripten_set_touchstart_callback, _emscripten_glGetPointerv,
    emscriptenWebGLComputeImageSize, _emscripten_glGetBooleanv, _emscripten_glClientActiveTexture,
    _emscripten_glVertexAttribDivisor, _cos, _emscripten_glCompressedTexImage2D,
    _emscripten_glUniform1i, _pthread_getspecific, _emscripten_glGenBuffers,
    _emscripten_glDeleteObjectARB, _emscripten_glTexImage2D, _emscripten_glVertexAttribPointer,
    _emscripten_glIsProgram, __isLeapYear, _emscripten_request_fullscreen_strategy,
    _emscripten_glBlendEquationSeparate, _log, _emscripten_glGetString, ___syscall220,
    _emscripten_glIsFramebuffer, ___cxa_atexit, _emscripten_glIsEnabled, _emscripten_glScissor,
    _emscripten_glFramebufferTexture2D, _emscripten_glTexParameteriv, _pthread_cleanup_push,
    _emscripten_set_element_css_size, _emscripten_glBindProgramARB, _emscripten_glStencilOpSeparate,
    _emscripten_glGetUniformfv, _glutCreateWindow, _emscripten_glFramebufferRenderbuffer,
    ___syscall140, _pthread_setspecific, ___syscall145, ___syscall146,
    _emscripten_glGetActiveAttrib, _emscripten_glAttachShader, _emscripten_glDrawRangeElements,
    _emscripten_get_now_is_monotonic, _emscripten_glUniform2i, _emscripten_glUniform2f,
    _emscripten_glTexParameterfv, _emscripten_glIsBuffer, _emscripten_glUniformMatrix2fv,
    _execvp, _atan2, _emscripten_glTexParameterf, _emscripten_glGetAttachedShaders,
    _emscripten_glGenTextures, _emscripten_glDrawArraysInstanced, ___cxa_find_matching_catch,
    _eglGetConfigAttrib, _emscripten_glClear, _emscripten_memcpy_big, ___cxa_guard_release,
    _glutInitDisplayMode, _emscripten_glValidateProgram, _emscripten_glVertexAttrib2fv,
    _emscripten_glUniform3f, _emscripten_glUniform4iv, _emscripten_glGetTexParameteriv, ___setErrNo,
    _eglGetProcAddress, _emscripten_glDrawElementsInstanced, _emscripten_glBindAttribLocation,
    _llvm_pow_f32, _emscripten_set_mousedown_callback, _emscripten_glDrawElements,
    _emscripten_set_canvas_size, _emscripten_glVertexAttrib2f, ___resumeException,
    _emscripten_glFlush, _emscripten_glPolygonOffset, _emscripten_glUniform4i,
    _emscripten_glCheckFramebufferStatus, _emscripten_glGetError, _emscripten_glClearDepthf,
    _nanosleep, _eglDestroySurface, _emscripten_force_exit, _emscripten_set_mouseenter_callback,
    _emscripten_glUniform3i, _emscripten_glRotatef, _emscripten_glDeleteShader, _pthread_once,
    _emscripten_glVertexAttrib1fv, _fabs, _emscripten_glReadPixels, _emscripten_glMatrixMode,
    _eglCreateWindowSurface, _emscripten_sleep_with_yield, _sin, _emscripten_glGetUniformLocation,
    emscriptenWebGLGet, _emscripten_get_device_pixel_ratio, _emscripten_set_mouseup_callback,
    _emscripten_glEnableVertexAttribArray, _eglWaitClient, _emscripten_get_now, ___syscall10,
    _emscripten_glNormalPointer, _emscripten_glHint, _emscripten_glTexCoordPointer,
    _emscripten_glEnable, __registerRestoreOldStyle, _getpwnam, ___lock,
    _emscripten_glBindFramebuffer, ___syscall6, ___syscall5, _emscripten_glStencilFuncSeparate,
    _emscripten_glVertexAttrib3f, _time, _pthread_mutex_lock, _gettimeofday,
    _emscripten_glVertexAttrib1f, _emscripten_glGetFramebufferAttachmentParameteriv, _eglWaitNative,
    _exit, _emscripten_asm_const_4, _emscripten_set_resize_callback, _emscripten_asm_const_2,
    _emscripten_asm_const_1, _emscripten_glEnableClientState, _pthread_cleanup_pop, _dlerror,
    _putenv, _emscripten_glClearStencil, _llvm_pow_f64, _emscripten_set_keypress_callback,
    _localtime_r, _emscripten_get_gamepad_status, _emscripten_glUniform4f, _emscripten_glUniform2fv,
    _eglChooseConfig, ___unlock, _pthread_key_create, _emscripten_glLoadMatrixf, _dlclose,
    _emscripten_glShaderBinary, _emscripten_glIsTexture, ___cxa_allocate_exception,
    _emscripten_glBlendFunc, _emscripten_get_num_gamepads, ___syscall221, _emscripten_glStencilMask,
    _emscripten_glUniform1iv, _emscripten_glGetVertexAttribPointerv,
    _emscripten_glCompressedTexSubImage2D, emscriptenWebGLGetUniform, _tzset, _fabsf,
    _emscripten_glUniform1fv, _emscripten_glGetActiveUniform, emscriptenWebGLGetVertexAttrib,
    ___syscall197, _emscripten_glUniform2iv, ___syscall195, _eglSwapInterval, _clock_gettime,
    _execlp, _emscripten_set_gamepadconnected_callback, _glutDestroyWindow, _strftime,
    _emscripten_glVertexAttrib4f, _emscripten_glGetVertexAttribiv, _emscripten_glTexSubImage2D,
    _emscripten_glDisable, _emscripten_glPixelStorei, ___syscall194, _emscripten_glUniformMatrix3fv,
    _emscripten_glDepthRange, _emscripten_set_mousemove_callback, _getenv, ___syscall33,
    _emscripten_glGetBufferParameteriv, _emscripten_glFinish, _pthread_cond_broadcast, ___syscall39,
    ___syscall38, _emscripten_glDepthMask, _emscripten_glDrawBuffers, _emscripten_glLineWidth,
    _emscripten_glDeleteProgram, _tan, _eglMakeCurrent, _emscripten_glGetObjectParameterivARB,
    _emscripten_exit_pointerlock, _emscripten_glVertexAttrib4fv, _emscripten_glClearDepth,
    _emscripten_glStencilFunc, _abort, _emscripten_glGetUniformiv, ___syscall183,
    _emscripten_glGetRenderbufferParameteriv, _ceil, _eglSwapBuffers, _emscripten_glGetShaderiv,
    _localtime, _emscripten_glGenFramebuffers, _floor, _emscripten_glUniformMatrix4fv,
    _emscripten_glLoadIdentity, _pthread_cond_wait, _emscripten_set_blur_callback,
    _emscripten_glUniform1f, emscriptenWebGLGetTexPixelData, _eglWaitGL,
    _emscripten_glIsRenderbuffer, ___buildEnvironment, _emscripten_glRenderbufferStorage,
    ___syscall40, ___cxa_pure_virtual, _sigaction, _emscripten_set_keydown_callback,
    _emscripten_glTexParameteri, _emscripten_set_touchcancel_callback, _emscripten_glCreateShader,
    ___syscall4, _emscripten_glCopyTexSubImage2D, _eglInitialize, _usleep,
    _emscripten_glDeleteTextures, _emscripten_glBindRenderbuffer, _emscripten_glBufferSubData,
    _emscripten_glGetShaderSource, _emscripten_set_touchend_callback, _emscripten_exit_fullscreen,
    _emscripten_glGetVertexAttribfv, _emscripten_get_element_css_size,
    _emscripten_request_pointerlock, _emscripten_glVertexAttrib3fv, _emscripten_cancel_main_loop,
    _emscripten_glCompileShader, _glClear, _emscripten_glFrustum, _emscripten_glSampleCoverage,
    _emscripten_glDepthRangef, __exit, _emscripten_glGetShaderInfoLog, _emscripten_glStencilOp,
    _eglGetDisplay, _emscripten_glGenerateMipmap, _emscripten_glColorMask,
    _emscripten_glLinkProgram, _emscripten_glBlendEquation, _emscripten_glBlendColor, _pthread_self,
    _pthread_mutex_unlock, _emscripten_glBindVertexArray, _sqrt, _emscripten_get_pointerlock_status,
    _emscripten_glActiveTexture, _emscripten_set_wheel_callback, _emscripten_glGenRenderbuffers,
    ___syscall54, _emscripten_glDeleteBuffers, _eglDestroyContext, _emscripten_glColorPointer,
    _emscripten_set_main_loop, _emscripten_glIsShader, _emscripten_glGetProgramInfoLog,
    _emscripten_glDeleteFramebuffers, _emscripten_glUniform4fv, ___cxa_throw,
    _emscripten_set_visibilitychange_callback, _emscripten_set_focus_callback, __arraySum,
    _emscripten_glClearColor, _emscripten_glBufferData, _emscripten_set_pointerlockchange_callback,
    _emscripten_glCreateProgram, _emscripten_glGenVertexArrays, _emscripten_glGetAttribLocation,
    _emscripten_glGetInfoLogARB, _emscripten_glDeleteRenderbuffers, _emscripten_glBindBuffer,
    _atexit, _emscripten_glGetFloatv, _emscripten_glDetachShader, _emscripten_asm_const_0,
    _emscripten_glBindTexture, _emscripten_do_request_fullscreen, STACKTOP, STACK_MAX,
    tempDoublePtr, ABORT, cttz_i8, ___dso_handle, _environ
  };
  ...
  const asm = require('./asm')(Module.asmGlobalArg, Module.asmLibraryArg, Module.buffer);

  // EMSCRIPTEN_END_ASM
  Module.runPostSets = asm.runPostSets;
  Module._main = asm._main;
  Module._malloc = _malloc = asm._malloc;
  ...
  Module._strstr = asm._strstr;
  Module._i64Add = asm._i64Add;
  Module._i64Subtract = asm._i64Subtract;
  Module._emscripten_GetProcAddress = asm._emscripten_GetProcAddress;
  ...
  Module.dynCall_iiiiiiii = asm.dynCall_iiiiiiii;
  Module.dynCall_iiiiiid = asm.dynCall_iiiiiid;
  Module.dynCall_vd = asm.dynCall_vd;
  Module.dynCall_vid = asm.dynCall_vid;
  ...
  Module.dynCall_viiiiiiiiiii = asm.dynCall_viiiiiiiiiii;
  Module.dynCall_viddd = asm.dynCall_viddd;
  Module.dynCall_iiiii = asm.dynCall_iiiii;
  Module.dynCall_vidd = asm.dynCall_vidd;
  ...
  Module.dynCall_viid = asm.dynCall_viid;
  Module.dynCall_vidddd = asm.dynCall_vidddd;
  ...
  Module.dynCall_iiiiidii = asm.dynCall_iiiiidii;
  Module.dynCall_iiiiiiiiii = asm.dynCall_iiiiiiiiii;
  Module.dynCall_vdddddd = asm.dynCall_vdddddd;
  Module.dynCall_vdddd = asm.dynCall_vdddd;
  Module.dynCall_vdd = asm.dynCall_vdd;
  Module.dynCall_v = asm.dynCall_v;
  Module.dynCall_vdi = asm.dynCall_vdi;
  Module.dynCall_iiiiid = asm.dynCall_iiiiid;
  ...
  Module.callMain = function callMain(args) {
    ...
    const initialEmtStackTop = asm.emtStackSave();
